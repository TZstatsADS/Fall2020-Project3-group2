...
## Last part: 
### Running the test images from Test_set on 11/04/2020 and its results 

### Step 1: import data

Provide directories for testing images. Testing images and Testing fiducial points will be in different subfolders. 
```{r, include=FALSE}
...
```{r}
#testset_label
info_test <- read.csv("../data/test_set/label.csv")
```

If you choose to extract features from images, such as using Gabor filter, R memory will exhaust all images are read together. The solution is to repeat reading a smaller batch(e.g 100) and process them. 
```{r}
test_files_m <- length(list.files(test_image_dir))
...
Fiducial points are stored in matlab format. In this step, we read them and store them in a list.
```{r read fiducial points for test_set, warning=FALSE, message=FALSE}
#function to read fiducial points
#input: index
#output: matrix of fiducial points corresponding to the index
readMat.matrix_test <- function(index){
     return(round(readMat(paste0(test_image_dir, sprintf("%04d", index), ".mat"))[[1]],0))
}
#load fiducial points
fiducial_pt_list_test <- lapply(1:test_files_m, readMat.matrix_test)
save(fiducial_pt_list_test, file="../output/fiducial_pt_list_test.RData")
```

### GBM final results
```{r test_set GBM model results}
#testset_label <- read.csv("../data/test_set/label.csv")

# extract feature from test_set save as feature_dat_testset and the run time tm_feature_testset
tm_feature_testset <- NA
run.feature_testset <- TRUE
if(run.feature_testset){
  tm_feature_testset <- system.time(dat_testset <- feature(fiducial_pt_list_test, testset_label$Index))
  # save test_set features and time for extracting test_set feature
  # save dat_testset as feature_dat_testset
  save(dat_testset, file="../output/feature_dat_testset.RData")
  saveRDS(tm_feature_testset, "../output/tm_feature_testset.RDS")
}else{
  load("../output/feature_dat_testset.RData")
  tm_feature_testset <- readRDS("../output/tm_feature_testset.RDS")
}

# run the gmb model for test_set
run.testset_gbm = TRUE
tm_testset_gbm <- NA
if(run.testset_gbm){
  tm_testset_gbm <- system.time(pred_testset_gbm <- test_gbm(gbm.fit.model = gbm.baseline, input.test = dat_testset, n = 50))
  save(pred_testset_gbm, file="../output/pred_testset_gbm.RData")
  saveRDS(tm_testset_gbm, "../output/tm_testset_gbm.RDS")
}else{
  load("../output/pred_testset_gbm.RData")
  tm_testset_gbm <- readRDS("../output/tm_testset_gbm.RDS")
}

# performance 
accuracy_gbm_testset <- mean(testset_label$label == pred_testset_gbm)
pred_testset_gbm_num <- as.numeric(pred_testset_gbm)
tpr.fpr <- WeightedROC(pred_testset_gbm_num, testset_label$label)
auc_test <- WeightedAUC(tpr.fpr)

# Confusion Matrix 
library(caret)
# may need to comment it out
confusionMatrix(testset_label$label, as.factor(pred_testset_gbm))

cat("The accuracy of model: GBM baseline is", mean(testset_label$label == pred_testset_gbm)*100, "%.\n")
cat("The AUC of model: GBM baseline is", auc_test, ".\n")
print(paste("Time for constructing training features=", tm_feature_train[1],  "s"))
print(paste("Time for constructing testing features=", tm_feature_testset[1], "s"))
print(paste("Time for training model=", tm_train_gbm_baseline[1], "s"))
print(paste("Time for testing model=", tm_testset_gbm[1], "s"))
```

### SVM final results
```{r test_set SVM model results}
# run the SVM model for test_set
run.testset_svm = TRUE
tm_testset_svm <- NA
if(run.testset_sv){
  tm_testset_svm <- system.time(pred_svm_testset <- predict(fit_train_svm, dat_testset))
  save(pred_svm_testset, file="../output/pred_svm_testset.RData")
  saveRDS(tm_testset_svm, "../output/tm_testset_svm.RDS")
}else{
  load("../output/pred_svm_testset.RData")
  tm_testset_svm <- readRDS("../output/tm_testset_svm.RDS")
}

# Evaluation SVM for the test_set
accu_svm_test <- mean(testset_label$label == pred_svm_testset)
real_label_test = testset_label$label %>% as.character() %>% as.numeric()
pred_value_svm_testset  = pred_svm_testset%>% as.character() %>% as.numeric()
confusionMatrix(pred_svm_testset, testset_label$label)
# The AUC for SVM model in the testing set
AUC_SVM_test = auc_roc(real_label_test, pred_value_svm_testset)
AUC_SVM_test

cat("The accuracy of model: cost =", cost_choose, "is", accu_svm_test * 100, "%.\n")
cat("The auc value for the SVM model is", AUC_SVM_test * 100, "%.\n")
Model_performace(tm_feature_train, tm_feature_testset, tm_train_svm, tm_testset_svm)
